...
\begin{frame}{Dataset Diverifikasi, Dataset Final Dibangun}
  \framesubtitle{Data mentah diproses menjadi dataset siap indeks.}
  \centering
  \begin{tikzpicture}[
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Temuan penting}
      \textit{medical\_specialty} multi-label (91,2\%), tidak valid sebagai \textit{ground-truth} relevansi $\Rightarrow$ qrels disusun terpisah (semi-otomatis, sadar-sinonim).
    \end{block}
  \end{columns}
...
\begin{frame}{BM25 pada MTSamples Sudah Diuji, tapi Sebagai Baseline Naif}
  ...
  \begin{block}{Gap yang diisi}
    Belum ada studi yang mengevaluasi lapisan normalisasi + ekspansi sinonim/singkatan klinis untuk BM25 pada MTSamples, apalagi yang diintegrasikan langsung ke OpenMRS.
  \end{block}
...
\begin{frame}{Lima Varian Retrieval (Ablation Study)}
  ...
  \end{block}
  \vspace{0.15cm}
  \begin{block}{Untuk apa 5 varian ini?}
    \scriptsize
    V0 = kontrol; V1 = istilah klinis dasar (negasi, singkatan); V2 = jembatani kesenjangan istilah dokter--dokumen (mis. ``CKD''); V3 = tahan typo/morfologi tanpa \textit{lexicon}; V4 = uji ekspansi otomatis vs. manual (V2). \textit{Dropdown} UI $=$ alat \textit{ablation study} interaktif, bukan 5 mode produksi setara.
  \end{block}
\end{frame}
