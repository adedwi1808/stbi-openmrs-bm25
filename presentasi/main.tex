...
% 3
\sectionframe{Data \& Dataset}

% 4
\begin{frame}{Dari 4.999 Baris ke 2.316 Dokumen}
  \centering
  \begin{tikzpicture}[
...
% 6
\begin{frame}{BM25 di MTSamples: Sebatas Baseline}
  \scriptsize
  \renewcommand{\arraystretch}{1.25}
  \begin{tabular}{|>{\raggedright\arraybackslash}p{0.29\textwidth}|>{\raggedright\arraybackslash}p{0.33\textwidth}|>{\raggedright\arraybackslash}p{0.27\textwidth}|}
    \hline
...
% 7
\begin{frame}{Urgensi Diukur Langsung pada Dataset}
  \small
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}{Lima Varian Retrieval (Ablation Study)}
  ...
  \begin{tabular}{|c|l|l|}
    ...
    \hline
  \end{tabular}
  \source{Parameter BM25 identik pada kelima varian ($k_1{=}1{,}2$, $b{=}0{,}75$); fungsi skor: Robertson \& Zaragoza (2009) [6].}
\end{frame}
...
% 12
\begin{frame}{Hasil Evaluasi Lima Varian}
  \centering
  \renewcommand{\arraystretch}{1.25}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Temuan}
      \scriptsize
      Selisih V1 vs V0 sangat kecil: nDCG +0,0033 dan P@10 +0,0020, setara \textbf{1 dokumen} dari 500 posisi terambil. Recall@10 justru turun. Belum ada uji signifikansi $\Rightarrow$ klaim yang aman: normalisasi klinis \textbf{tidak menurunkan} kualitas, dengan respons 3,6$\times$ lebih cepat (0,68 vs 2,48 ms).
    \end{block}
    \column{0.48\textwidth}
...
\begin{frame}{Kesimpulan}
  \begin{block}{Apa yang dicapai}
    \begin{itemize}
      ...
      \item Hasil: V1 unggul tipis atas baseline naif, namun selisihnya dalam rentang \textit{noise} $\Rightarrow$ klaim dibatasi pada ``tidak menurunkan kualitas, respons lebih cepat''.
      \item Keterbatasan qrels (metadata vs transkripsi) diidentifikasi dan menjelaskan sebagian besar penurunan varian ekspansi.
