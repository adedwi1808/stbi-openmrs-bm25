...
\begin{frame}{Dataset Proyek Beralih ke MTSamples}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \end{block}
  \end{columns}
  \source{Sumber: MTSamples Medical Transcriptions, Kaggle \texttt{tboyle10/medicaltranscriptions} [1].}
\end{frame}
...
\begin{frame}{MTSamples Menyediakan Korpus Teks Medis yang Siap Diindeks}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Spesialisasi terbanyak (baris mentah)}
      \begin{tabular}{@{}lr@{}}
        Surgery & 1.103 \\
    ...
    \begin{block}{Setelah pembersihan korpus}
      \small
      2.609 baris duplikat digabung dan disaring ($\geq$50 token) $\rightarrow$ korpus final \textbf{2.316 dokumen}. \textit{medical\_specialty} multi-label (91,2\%), \textbf{tidak} layak jadi \textit{ground-truth} relevansi.
    \end{block}
  \end{columns}
  \source{Sumber: pengolahan dataset MTSamples [1] (\texttt{tools/build\_corpus.py}, \texttt{corpus\_stats.txt}).}
\end{frame}
...
\begin{frame}{BM25 pada MTSamples Sudah Diuji, tapi Sebagai Baseline Naif}
  \framesubtitle{Penelitian terkait belum menyasar optimasi tokenisasi BM25 untuk teks klinis.}
  \scriptsize
...
\begin{frame}{Tokenisasi Generik Berisiko Membuang Sinyal Klinis Penting}
  \framesubtitle{Urgensi: baseline lowercase + strip tanda baca tidak cukup untuk transkripsi medis.}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{0.31\textwidth}
...
\begin{frame}{BM25 Memberi Skor Berdasarkan Istilah dan Panjang Dokumen}
  ...
  \vspace{0.25cm}
  \begin{center}
    \keyword{Parameter yang diuji:} $k_1$, $b$, dan strategi tokenisasi \qquad \keyword{Baseline:} TF-IDF \& BM25 tokenisasi naif
  \end{center}
  \source{Sumber: Robertson \& Zaragoza (2009) [6].}
\end{frame}
...
\begin{frame}{Evaluasi Mengukur Relevansi Ranking dan Respons Sistem}
  ...
  \begin{block}{Rancangan uji}
    Bangun \textit{query set} dan \textit{relevance judgment} (bukan dari \textit{medical\_specialty}, karena bersifat multi-label); tuning $k_1$ dan $b$; lalu bandingkan BM25 tokenisasi klinis terhadap baseline TF-IDF dan BM25 tokenisasi naif pada korpus MTSamples.
  \end{block}
\end{frame}
