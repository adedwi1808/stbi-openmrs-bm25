...
\end{center}
\vspace{0.4cm}
\noindent Pencarian pada Rekam Medis Elektronik (RME) konvensional umumnya terbatas pada pencocokan kata kunci eksak, sehingga gagal menangani sinonim, singkatan, dan variasi terminologi medis pada catatan klinis teks bebas. Proyek ini mengimplementasikan mesin \textit{ranked retrieval} berbasis BM25 dan mengintegrasikannya ke dalam platform open-source OpenMRS sebagai modul tambahan. Kontribusi utamanya bukan sekadar penerapan BM25, melainkan studi ablasi terhadap lapisan \textbf{normalisasi dan ekspansi klinis}: stopword list yang mempertahankan penanda negasi, deteksi istilah majemuk, serta ekspansi singkatan dan sinonim medis. Sistem dievaluasi pada korpus MTSamples yang diproses menjadi 2.316 dokumen final, dengan lima varian retrieval (V0--V4) yang dibandingkan berdampingan. Hasil menunjukkan bahwa normalisasi klinis (V1) meningkatkan nDCG@10 dari 0,6832 menjadi 0,6865 dibanding baseline naif (V0), sementara varian ekspansi menunjukkan \textit{trade-off} presisi yang dilaporkan apa adanya. Modul yang dihasilkan diuji berjalan pada OpenMRS 2.8.8 dengan indeks Apache Lucene 8.11.2 yang di-\textit{shade} untuk menghindari konflik dependensi.
...
\subsection{Batasan}
Batasan proyek ini adalah: (1) platform yang digunakan adalah OpenMRS Reference Application, bukan pengembangan RME dari nol; (2) algoritma yang diimplementasikan adalah BM25, tidak mencakup pendekatan \textit{neural retrieval}; (3) dataset yang digunakan adalah MTSamples yang memuat sekitar 5.000 transkripsi medis; (4) modul diintegrasikan melalui endpoint HTTP tanpa mengubah skema basis data inti OpenMRS; dan (5) evaluasi dilakukan secara teknis menggunakan metrik IR standar tanpa uji klinis dengan pasien nyata.
...
Kajian pustaka lanjutan menunjukkan bahwa BM25 pada korpus MTSamples sudah pernah diuji, namun selalu diperlakukan sebagai \textit{baseline} naif, bukan sebagai objek optimasi tokenisasi. Tabel~\ref{tab:gap} merangkum perbandingannya.

\begin{table}[h]
  \centering
  \caption{Perbandingan studi terkait dan perlakuan terhadap BM25.}
  \label{tab:gap}
  \begin{tabular}{@{}p{3.9cm}p{5.0cm}p{4.4cm}@{}}
    \toprule
    \textbf{Studi} & \textbf{Fokus} & \textbf{Perlakuan terhadap BM25} \\
    \midrule
    Mikkelsen (2026) & Benchmark \textit{embedding} vs BM25 & Baseline dengan tokenisasi lowercase + hapus tanda baca \\
    Zhang (2021) & Optimasi bobot skor BM25 & Perbaikan di fungsi skor, bukan tokenisasi \\
    Gayen dkk. (2023) & Porting tokenisasi Essie ke Solr & Tokenisasi klinis, bukan untuk BM25 di RME \\
    Griffon dkk. (2012) & Ekspansi sinonim UMLS & Untuk PubMed, bukan untuk RME \\
    \bottomrule
  \end{tabular}
\end{table}

Berdasarkan perbandingan tersebut, gap yang diisi proyek ini adalah belum adanya studi yang secara khusus mengevaluasi lapisan normalisasi dan ekspansi sinonim/singkatan klinis untuk algoritma BM25 pada korpus MTSamples, apalagi yang diintegrasikan langsung ke OpenMRS.
...
\subsection{Pembangunan Korpus dan Lexicon}
Korpus final berisi 2.316 dokumen dari 4.999 baris mentah. Statistiknya dirangkum pada Tabel~\ref{tab:korpus}. Temuan penting adalah 91,2\% dokumen bersifat multi-spesialisasi, sehingga kolom \texttt{medical\_specialty} tidak dapat dijadikan \textit{ground-truth} relevansi untuk evaluasi.

\begin{table}[h]
  \centering
  \caption{Statistik pembangunan korpus final.}
  \label{tab:korpus}
  \begin{tabular}{@{}l r@{}}
    \toprule
...
Lexicon klinis yang dibangun beserta cakupannya pada korpus disajikan pada Tabel~\ref{tab:lexicon}. Pengukuran dilakukan langsung pada \texttt{corpus.jsonl} untuk memastikan urgensi tokenisasi klinis berbasis bukti, bukan asumsi.

\begin{table}[h]
  \centering
  \caption{Komponen lexicon klinis dan cakupan pada korpus (2.316 dokumen).}
  \label{tab:lexicon}
  \begin{tabular}{@{}l c c@{}}
    \toprule
...
Mesin IR ditulis dalam Java dengan Apache Lucene 8.11.2. Karena OpenMRS membundel Lucene versi lama (tanpa BM25), Lucene 8 di-\textit{shade} dengan relokasi paket ke \path|org.openmrs.module.irbm25.shaded.lucene| agar tidak bentrok. Indeks dibangun secara multi-field (\texttt{content\_naive}, \texttt{content\_clinical}, \texttt{content\_ngram}) sehingga kelima varian dapat dibandingkan secara adil pada satu korpus dengan parameter BM25 yang identik.
...
yang mengembalikan dokumen terurut beserta skor dan \textit{snippet}. Antarmuka pengguna menyediakan \textit{dropdown} untuk memilih varian serta mode ``bandingkan semua'' yang menampilkan kelima varian berdampingan. Link navigasi ditambahkan ke \textit{sidebar} Legacy UI melalui extension \texttt{org.openmrs.gutter.tools}. Gambar~\ref{fig:ui} menampilkan tangkapan layar mode perbandingan.
...
Hasil menunjukkan bahwa V1 (normalisasi klinis) sedikit mengungguli baseline naif V0 pada P@10 dan nDCG@10, yang mendukung kontribusi lapisan tokenisasi klinis. Sementara itu, V2 dan V4 menurunkan presisi pada qrels ini, yang merupakan \textit{trade-off} wajar dari ekspansi; hal ini dilaporkan apa adanya dan menjadi arah untuk penyusunan qrels manual oleh klinisi pada tahap berikutnya. Secara fungsional, ekspansi singkatan terverifikasi bekerja: \textit{query} ``CKD'' berhasil menemukan dokumen berjudul ``Chronic Kidney Disease - Followup'', dan \textit{query} ``hypertension'' menemukan dokumen yang memuat singkatan ``HTN''.

\section{Kesimpulan}
Proyek ini berhasil mengimplementasikan mesin \textit{ranked retrieval} BM25 dengan lapisan normalisasi dan ekspansi klinis, mengintegrasikannya ke OpenMRS sebagai modul tambahan, serta mengevaluasinya melalui ablation study lima varian pada korpus MTSamples. Validasi berbasis korpus menunjukkan bahwa penanda negasi, istilah majemuk, dan singkatan medis muncul pada sebagian besar dokumen, sehingga tokenisasi klinis bukan sekadar asumsi. Hasil evaluasi menunjukkan perbaikan kecil namun konsisten dari normalisasi klinis (V1) dibanding baseline naif, sekaligus mengungkap \textit{trade-off} presisi pada varian ekspansi yang perlu ditindaklanjuti dengan \textit{relevance judgment} manual. Modul teruji berjalan pada OpenMRS 2.8.8 dan siap digunakan sebagai purwarupa pencarian cerdas bagi tenaga medis.
